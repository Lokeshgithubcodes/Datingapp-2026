...
public class MessageHub(IUnitOfWork uow, IHubContext<PresenceHub> presenceHub) : Hub
{
    ...
    {
        var sender = await uow.MemberRepository.GetMemberIdAsync(GetUserId());
        var recipient = await uow.MemberRepository.GetMemberIdAsync(createMessageDto.RecipientId);

        if(recipient == null || sender == null || sender.Id == createMessageDto.RecipientId)
            throw new HubException("Cannot send messages");
        ...
        var userInGroup = group != null && group.Connections.Any(x => x.UserId 
            == message.RecipientId);
